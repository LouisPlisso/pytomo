...
from . import set as dns_set

# define SimpleSet here for backwards compatibility
SimpleSet = dns_set.Set

class DifferingCovers(dns_exception.DNSException):
    """Raised if an attempt is made to add a SIG/RRSIG whose covered type
    is not the same as that of the other rdatas in the rdataset."""
    pass

class IncompatibleTypes(dns_exception.DNSException):
    """Raised if an attempt is made to add rdata of an incompatible type."""
    pass

class Rdataset(dns_set.Set):
    """A DNS rdataset.
    ...
    def __init__(self, rdclass, rdtype, covers=dns_rdatatype.NONE):
    ...
    def add(self, rd, ttl=None):
        ...
        if not ttl is None:
            self.update_ttl(ttl)
        if self.rdtype == dns_rdatatype.RRSIG or \
           self.rdtype == dns_rdatatype.SIG:
            covers = rd.covers()
            if len(self) == 0 and self.covers == dns_rdatatype.NONE:
                self.covers = covers
            elif self.covers != covers:
                raise DifferingCovers
        if dns_rdatatype.is_singleton(rd.rdtype) and len(self) > 0:
            self.clear()
    ...
    def update(self, other):
        ...
        @type other: dns_rdataset.Rdataset object"""
    ...
    def __repr__(self):
        ...
        return '<DNS ' + dns_rdataclass.to_text(self.rdclass) + ' ' + \
               dns_rdatatype.to_text(self.rdtype) + ctext + ' rdataset>'
    ...
    def to_text(self, name=None, origin=None, relativize=True,
        ...
        if len(self) == 0:
            ...
            print >> s, '%s%s%s %s' % (ntext, pad,
                                       dns_rdataclass.to_text(rdclass),
                                       dns_rdatatype.to_text(self.rdtype))
        else:
            for rd in self:
                print >> s, '%s%s%d %s %s %s' % \
                      (ntext, pad, self.ttl, dns_rdataclass.to_text(rdclass),
                       dns_rdatatype.to_text(self.rdtype),
